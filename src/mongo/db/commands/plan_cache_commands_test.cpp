...
namespace {
    ...
    TEST(PlanCacheCommandsTest, Canonicalize) {
        ...
        ASSERT_NOT_OK(PlanCacheCommand::canonicalize(&txn, ns,
                                                     fromjson("{query: {}, sort: 1}")).getStatus());
        // Bad query (invalid sort order)
        ASSERT_NOT_OK(PlanCacheCommand::canonicalize(&txn, ns,
            fromjson("{query: {}, sort: {a: 0}}")).getStatus());

        // Valid parameters
        auto statusWithCQ = PlanCacheCommand::canonicalize(&txn, ns,
                                                    fromjson("{query: {a: 1, b: 1}}"));
        ASSERT_OK(statusWithCQ.getStatus());
        ...
        statusWithCQ = PlanCacheCommand::canonicalize(&txn, ns,
            fromjson("{query: {a: 1, b: 1}, sort: {a: 1, b: 1}}"));
        ASSERT_OK(statusWithCQ.getStatus());
        ...
        statusWithCQ = PlanCacheCommand::canonicalize(&txn, ns,
            fromjson("{query: {a: 1, b: 1}, sort: {aab: 1}}"));
        ASSERT_OK(statusWithCQ.getStatus());
        ...
        statusWithCQ = PlanCacheCommand::canonicalize(&txn, ns,
            fromjson("{query: {b: 3, a: 3}, sort: {a: 1, b: 1}}"));
        ASSERT_OK(statusWithCQ.getStatus());
